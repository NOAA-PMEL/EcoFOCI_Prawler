...
if args.met_rudix:
    ...
    kid = KetchMetData.get_data(datetime.datetime(2017,4,26))
...
if args.image:
    ...
    cs = plt.imshow(np.transpose(mesh_grid_t), extent=extent, cmap=cmocean.cm.thermal, vmin=-2.0, vmax=15.0, aspect='auto', alpha=0.85)
    cs.cmap.set_under('w')
    ax.xaxis.set_major_locator(DayLocator(bymonthday=15))
    ax.xaxis.set_minor_locator(DayLocator(bymonthday=[5,10,15,20,25,30]))
    ax.xaxis.set_major_formatter(plt.NullFormatter())
